...
class ExternalSort:
    def get_full_animation_steps(self, input_file):
        ...
        # --- Pass 0: Tao Initial Runs [cite: 254, 587] ---
        all_runs = []
        ...
        # --- Merge Passes: Tron cho den khi xong [cite: 256, 590] ---
        pass_idx = 1
        while len(all_runs) > 1:
            ...
            while i < len(all_runs):
                if i + 1 < len(all_runs):
                    ...
                    steps.append({'act': 'SAVE_MERGED', 'values': merged, 'pass_idx': pass_idx, 'desc': f"Hoan tat Pass {pass_idx}"})
                    i += 2
                else:
