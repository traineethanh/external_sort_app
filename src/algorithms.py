...
class ExternalSort:
    def get_full_animation_steps(self, input_file):
        ...
        run_size = 4  # Gia su moi Run ban dau chua 4 trang [cite: 254]
        steps = []
        
        steps.append({'act': 'INIT_DISK', 'values': data, 'desc': "Khoi tao du lieu goc tren Disk"})

        # --- PASS 0: Tao cac Run ban dau (Split & Sort) ---
        all_runs = []
        ...
        # --- MERGE PASSES: Tron cac Run (Pass 1, 2...) ---
        pass_idx = 1
        while len(all_runs) > 1:
            ...
            for i in range(0, len(all_runs), 2):
                if i + 1 < len(all_runs):
                    ...
                    steps.append({'act': 'SAVE_MERGED', 'values': merged, 'desc': f"Ghi ket qua Pass {pass_idx} (Run moi dai hon)"})
                else:
                    new_runs.append(all_runs[i])
                    steps.append({'act': 'SKIP_LE', 'run_idx': i, 'desc': "Run le, doi de tron o vong sau"})
            all_runs = new_runs
            pass_idx += 1
        return steps